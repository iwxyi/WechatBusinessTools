...
QString readTextFileAutoCodec(QString path, QString* usedCodec)
{
    ...
    {
        text = QTextCodec::codecForName("GBK")->toUnicode(ba.constData(), ba.size(), &state);
        if (usedCodec)
            *usedCodec = "GBK";
    }
    else
...
QString readTextFileWithFolder(QString path, QString folder, QTextCodec* codec)
{
    QString full = readTextFile(path, codec);
...
bool openFolderInExplorer(const QString &path)
{
