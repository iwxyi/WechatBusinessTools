...
QString ArabToCN(qint64 num)
{
    ...
    return des.replace("二千", "两千").replace("二万", "两万").replace("二亿", "两亿");
}

qint64 CNToArab(QString text)
{
    int len = text.length();
    text.replace("两", "二");
    qint64 val = 0;
 ...
 * 字符串转JSON字符串的value
 */
QString toJsonStringValue(QString text)
{
    return text.replace("\\", "\\\\")
            .replace("\"", "\\\"")
            .replace("\n", "\\n")
 ...
 * 字符串转MD5
 */
QByteArray toMd5(const QByteArray &ba)
{
...
{
    if (s1.toLower().startsWith("v"))
        s1 = s1.right(s1.length() - 1);
    if (s2.toLower().startsWith("v"))
 ...
 * 一般是隐私信息
 */
QString markText(QString text, int left, int right)
{
    if (text.length() <= left + right)
        return text;
    QString ans = text.left(left);
    for (int i = left; i < text.length() - right; i++)
        ans += "*";
    ans += text.right(right);
    return ans;
}

QString uniformNewlineCharacter(QString text)
{
    int nr_pos = text.indexOf("\n\r");
    int rn_pos = text.indexOf("\r\n");
    if (nr_pos > -1 && rn_pos > -1)
    {
        if (nr_pos < rn_pos)
            text = text.replace("\n\r", "\n");
        else
